...
Programa usado para codificar las entradas en las clausulas respectivas en
formato CNF, que seran pasados al sat-solver(minisat, etc).
*/
#include <iostream>
#include <stdlib.h>
#include <iostream>
...
//Funcion que calcula el numero de la variable del segmento.
...
//Funcion que calcula el numero de la variable de la celda.
int z(int i, int j){
    return (i + (j - 1) * n + seg);
}

//Funcion que calcula el numero de la variable de relacion.
...
int main(int argc, char const *argv[])
{
    ...
    for (int j = 2; j < m; ++j){
        for (int i = 2; i < n; ++i){
            ...
            //Evaluacion de celdas no bordes de la clausula tipo 5
            myfile << "c Tipo 5\n";
            ...
            myfile << -q(i,j,'n') << ' ' << -q(i-1,j,'n') << ' ' << -q(i-1,j,'e') << " 0\n";
            //[-q(i,j,n) v -q(i-1,j,e) v -q(i,j-1,w)]
            myfile << -q(i,j,'n') << ' ' << -q(i-1,j,'e') << ' ' << -q(i,j-1,'w') << " 0\n";
            //[-q(i,j,n) v -q(i-1,j,n) v -q(i,j-1,w)]
            myfile << -q(i,j,'n') << ' ' << -q(i-1,j,'n') << ' ' << -q(i,j-1,'w') << " 0\n";


            //[-q(i,j,n) v -q(i+1,j,n) v -q(i+1,j,w)]
            myfile << -q(i,j,'n') << ' ' << -q(i+1,j,'n') << ' ' << -q(i+1,j,'w') << " 0\n";
            //[-q(i,j,n) v -q(i+1,j,n) v -q(i,j-1,e)]
            myfile << -q(i,j,'n') << ' ' << -q(i+1,j,'n') << ' ' << -q(i,j-1,'e') << " 0\n";
            //[-q(i,j,n) v -q(i+1,j,w) v -q(i,j-1,e)]
            myfile << -q(i,j,'n') << ' ' << -q(i+1,j,'w') << ' ' << -q(i,j-1,'e') << " 0\n";
            ...
            myfile << -q(i,j,'s') << ' ' << -q(i-1,j,'s') << ' ' << -q(i-1,j,'e') << " 0\n";
            //[-q(i,j,s) v -q(i-1,j,e) v -q(i,j+1,w)]
            myfile << -q(i,j,'s') << ' ' << -q(i-1,j,'e') << ' ' << -q(i,j+1,'w') << " 0\n";
            //[-q(i,j,s) v -q(i-1,j,s) v -q(i,j+1,w)]
            myfile << -q(i,j,'s') << ' ' << -q(i-1,j,'s') << ' ' << -q(i,j+1,'w') << " 0\n";

            //[-q(i,j,s) v -q(i+1,j,s) v -q(i+1,j,w)]
            myfile << -q(i,j,'s') << ' ' << -q(i+1,j,'s') << ' ' << -q(i+1,j,'w') << " 0\n";
            //[-q(i,j,s) v -q(i+1,j,s) v -q(i,j+1,e)]
            myfile << -q(i,j,'s') << ' ' << -q(i+1,j,'s') << ' ' << -q(i,j+1,'e') << " 0\n";
            //[-q(i,j,s) v -q(i+1,j,w) v -q(i,j+1,e)]
            myfile << -q(i,j,'s') << ' ' << -q(i+1,j,'w') << ' ' << -q(i,j+1,'e') << " 0\n";
            ...
            myfile << -q(i,j,'w') << ' ' << -q(i,j-1,'s') << ' ' << -q(i,j-1,'w') << " 0\n";
            //[-q(i,j,w) v -q(i,j-1,w) v -q(i-1,j,n)]
            myfile << -q(i,j,'w') << ' ' << -q(i,j-1,'w') << ' ' << -q(i-1,j,'n') << " 0\n";
            //[-q(i,j,w) v -q(i,j-1,s) v -q(i-1,j,n)]
            myfile << -q(i,j,'w') << ' ' << -q(i,j-1,'s') << ' ' << -q(i-1,j,'n') << " 0\n";
            
            //[-q(i,j,w) v -q(i,j+1,n) v -q(i,j+1,w)]
            myfile << -q(i,j,'w') << ' ' << -q(i,j+1,'n') << ' ' << -q(i,j+1,'w') << " 0\n";
            //[-q(i,j,w) v -q(i,j+1,n) v -q(i-1,j,s)]
            myfile << -q(i,j,'w') << ' ' << -q(i,j+1,'n') << ' ' << -q(i-1,j,'s') << " 0\n";
            //[-q(i,j,w) v -q(i,j+1,w) v -q(i-1,j,s)]
            myfile << -q(i,j,'w') << ' ' << -q(i,j+1,'w') << ' ' << -q(i-1,j,'s') << " 0\n";
            ...
            myfile << -q(i,j,'e') << ' ' << -q(i,j-1,'s') << ' ' << -q(i,j-1,'e') << " 0\n";
            //[-q(i,j,e) v -q(i,j-1,e) v -q(i+1,j,n)]
            myfile << -q(i,j,'e') << ' ' << -q(i,j-1,'e') << ' ' << -q(i+1,j,'n') << " 0\n";
            //[-q(i,j,e) v -q(i,j-1,s) v -q(i+1,j,n)]
            myfile << -q(i,j,'e') << ' ' << -q(i,j-1,'s') << ' ' << -q(i+1,j,'n') << " 0\n";
            
            //[-q(i,j,e) v -q(i,j+1,n) v -q(i,j+1,e)]
            myfile << -q(i,j,'e') << ' ' << -q(i,j+1,'n') << ' ' << -q(i,j+1,'e') << " 0\n";
            //[-q(i,j,e) v -q(i,j+1,n) v -q(i+1,j,s)]
            myfile << -q(i,j,'e') << ' ' << -q(i,j+1,'n') << ' ' << -q(i+1,j,'s') << " 0\n";
            //[-q(i,j,e) v -q(i,j+1,e) v -q(i+1,j,s)]
            myfile << -q(i,j,'e') << ' ' << -q(i,j+1,'e') << ' ' << -q(i+1,j,'s') << " 0\n";
            //-
    ...
    for (int j = 2; j < m; j++ ){
        ...
        //Celdas del borde izquierdo de la clausula tipo 5
        ...
        //[-q(1,j,w) v -q(1,j+1,n) v -q(1,j+1,w)]
        myfile << -q(1,j,'w') << ' ' << -q(1,j+1,'n') << ' ' << -q(1,j+1,'w') << " 0\n";
        ...
        //[-q(1,j,n) v -q(1,j,w) v -q(1,j-1,w)]
        myfile << -q(1,j,'n') << ' ' << -q(1,j,'w') << ' ' << -q(1,j-1,'w') << " 0\n";
        
        //[-q(1,j,n) v -q(1,j,e) v -q(2,j,n)]
        myfile << -q(1,j,'n') << ' ' << -q(1,j,'e') << ' ' << -q(2,j,'n') << " 0\n";
        //[-q(1,j,n) v -q(1,j,e) v -q(1,j-1,e)]
        myfile << -q(1,j,'n') << ' ' << -q(1,j,'e') << ' ' << -q(1,j-1,'e') << " 0\n";
        //[-q(1,j,n) v -q(2,j,n) v -q(1,j-1,e)]
        myfile << -q(1,j,'n') << ' ' << -q(2,j,'n') << ' ' << -q(1,j-1,'e') << " 0\n";
        ...
        myfile << -q(1,j,'e') << ' ' << -q(1,j-1,'s') << ' ' << -q(1,j-1,'e') << " 0\n";
        //[-q(1,j,e) v -q(1,j-1,e) v -q(2,j,n)]
        myfile << -q(1,j,'e') << ' ' << -q(1,j-1,'e') << ' ' << -q(2,j,'n') << " 0\n";
        //[-q(1,j,e) v -q(1,j-1,s) v -q(2,j,n)]
        myfile << -q(1,j,'e') << ' ' << -q(1,j-1,'s') << ' ' << -q(2,j,'n') << " 0\n";
        
        //[-q(1,j,e) v -q(1,j+1,n) v -q(1,j+1,e)]
        myfile << -q(1,j,'e') << ' ' << -q(1,j+1,'n') << ' ' << -q(1,j+1,'e') << " 0\n";
        //[-q(1,j,e) v -q(1,j+1,n) v -q(2,j,s)]
        myfile << -q(1,j,'e') << ' ' << -q(1,j+1,'n') << ' ' << -q(2,j,'s') << " 0\n";
        //[-q(1,j,e) v -q(1,j+1,e) v -q(2,j,s)]
        myfile << -q(1,j,'e') << ' ' << -q(1,j+1,'e') << ' ' << -q(2,j,'s') << " 0\n";
        ...
        //[-q(1,j,s) v -q(1,j,w) v -q(1,j+1,w)]
        myfile << -q(1,j,'s') << ' ' << -q(1,j,'w') << ' ' << -q(1,j+1,'w') << " 0\n";
        
        //[-q(1,j,s) v -q(1,j,e) v -q(2,j,s)]
        myfile << -q(1,j,'s') << ' ' << -q(1,j,'e') << ' ' << -q(2,j,'s') << " 0\n";
        //[-q(1,j,s) v -q(1,j,e) v -q(1,j+1,e)]
        myfile << -q(1,j,'s') << ' ' << -q(1,j,'e') << ' ' << -q(1,j+1,'e') << " 0\n";
        //[-q(1,j,s) v -q(2,j,s) v -q(1,j+1,e)]
        myfile << -q(1,j,'s') << ' ' << -q(2,j,'s') << ' ' << -q(1,j+1,'e') << " 0\n";
        

        //Celdas del borde derecho de la clausula tipo 5
        ...
        //[-q(N,j,e) v -q(N,j+1,n) v -q(N,j+1,e)]
        myfile << -q(n,j,'e') << ' ' << -q(n,j+1,'n') << ' ' << -q(n,j+1,'e') << " 0\n";
        ...
        myfile << -q(n,j,'w') << ' ' << -q(n,j-1,'s') << ' ' << -q(n,j-1,'w') << " 0\n";
        //[-q(N,j,w) v -q(N,j-1,w) v -q(N-1,j,n)]
        myfile << -q(n,j,'w') << ' ' << -q(n,j-1,'w') << ' ' << -q(n-1,j,'n') << " 0\n";
        //[-q(N,j,w) v -q(N,j-1,s) v -q(N-1,j,n)]
        myfile << -q(n,j,'w') << ' ' << -q(n,j-1,'s') << ' ' << -q(n-1,j,'n') << " 0\n";
        
        //[-q(N,j,w) v -q(N,j+1,n) v -q(N,j+1,w)]
        myfile << -q(n,j,'w') << ' ' << -q(n,j+1,'n') << ' ' << -q(n,j+1,'w') << " 0\n";
        //[-q(N,j,w) v -q(N,j+1,n) v -q(N-1,j,s)]
        myfile << -q(n,j,'w') << ' ' << -q(n,j+1,'n') << ' ' << -q(n-1,j,'s') << " 0\n";
        //[-q(N,j,w) v -q(N,j+1,w) v -q(N-1,j,s)]
        myfile << -q(n,j,'w') << ' ' << -q(n,j+1,'w') << ' ' << -q(n-1,j,'s') << " 0\n";
    ...
    for (int i = 2; i < n; i++ ){
        //Celdas del borde Superior de la clausula tipo 5
        ...
        //[-q(i,1,n) v -q(i+1,1,n) v -q(i+1,1,w)]
        myfile << -q(i,1,'n') << ' ' << -q(i+1,1,'n') << ' ' << -q(i+1,1,'w') << " 0\n";
        ...
        //[-q(i,1,e) v -q(i,1,n) v -q(i+1,1,n)]
        myfile << -q(i,1,'e') << ' ' << -q(i,1,'n') << ' ' << -q(i+1,1,'n') << " 0\n";
        
        //[-q(i,1,e) v -q(i,1,s) v -q(i,2,e)]
        myfile << -q(i,1,'e') << ' ' << -q(i,1,'s') << ' ' << -q(i,2,'e') << " 0\n";
        //[-q(i,1,e) v -q(i,1,s) v -q(i+1,1,s)]
        myfile << -q(i,1,'e') << ' ' << -q(i,1,'s') << ' ' << -q(i+1,1,'s') << " 0\n";
        //[-q(i,1,e) v -q(i,2,e) v -q(i+1,1,s)]
        myfile << -q(i,1,'e') << ' ' << -q(i,2,'e') << ' ' << -q(i+1,1,'s') << " 0\n";
        ...
        myfile << -q(i,1,'s') << ' ' << q(i+1,1,'w') << ' ' << q(i+1,1,'s') << ' ' << q(i,2,'e') << " 0\n";
        
        //[-q(i,1,s) v -q(i-1,1,e) v -q(i-1,1,s)]
        myfile << -q(i,1,'s') << ' ' << -q(i-1,1,'e') << ' ' << -q(i-1,1,'s') << " 0\n";
        //[-q(i,1,s) v -q(i-1,1,s) v -q(i,2,w)]
        myfile << -q(i,1,'s') << ' ' << -q(i-1,1,'s') << ' ' << -q(i,2,'w') << " 0\n";
        //[-q(i,1,s) v -q(i-1,1,e) v -q(i,2,w)]
        myfile << -q(i,1,'s') << ' ' << -q(i-1,1,'e') << ' ' << -q(i,2,'w') << " 0\n";

        //[-q(i,1,s) v -q(i+1,1,w) v -q(i+1,1,s)]
        myfile << -q(i,1,'s') << ' ' << -q(i+1,1,'w') << ' ' << -q(i+1,1,'s') << " 0\n";
        //[-q(i,1,s) v -q(i+1,1,w) v -q(i,2,e)]
        myfile << -q(i,1,'s') << ' ' << -q(i+1,1,'w') << ' ' << -q(i,2,'e') << " 0\n";
        //[-q(i,1,s) v -q(i+1,1,s) v -q(i,2,e)]
        myfile << -q(i,1,'s') << ' ' << -q(i+1,1,'s') << ' ' << -q(i,2,'e') << " 0\n";
        ...
        //[-q(i,1,w) v -q(i,1,n) v -q(i-1,1,n)]
        myfile << -q(i,1,'w') << ' ' << -q(i,1,'n') << ' ' << -q(i-1,1,'n') << " 0\n";

        //[-q(i,1,w) v -q(i,2,n) v -q(i,2,w)]
        myfile << -q(i,1,'w') << ' ' << -q(i,2,'n') << ' ' << -q(i,2,'w') << " 0\n";
        //[-q(i,1,w) v -q(i,2,n) v -q(i-1,1,s)]
        myfile << -q(i,1,'w') << ' ' << -q(i,2,'n') << ' ' << -q(i-1,1,'s') << " 0\n";
        //[-q(i,1,w) v -q(i,2,w) v -q(i-1,1,s)]
        myfile << -q(i,1,'w') << ' ' << -q(i,2,'w') << ' ' << -q(i-1,1,'s') << " 0\n";
        
        
        //Celdas del borde inferior de la clausula tipo 5
        ...
        //[-q(i,M,s) v -q(i+1,M,s) v -q(i+1,M,w)]
        myfile << -q(i,m,'s') << ' ' << -q(i+1,m,'s') << ' ' << -q(i+1,m,'w') << " 0\n";
        ...
        //[-q(i,M,n) v -q(i+1,M,w) v -q(i+1,M,n)]
        myfile << -q(i,m,'n') << ' ' << -q(i+1,m,'w') << ' ' << -q(i+1,m,'n') << " 0\n";
        //[-q(i,M,n) v -q(i+1,M,w) v -q(i,M-1,e)]
        myfile << -q(i,m,'n') << ' ' << -q(i+1,m,'w') << ' ' << -q(i,m-1,'e') << " 0\n";
        //[-q(i,M,n) v -q(i+1,M,n) v -q(i,M-1,e)]
        myfile << -q(i,m,'n') << ' ' << -q(i+1,m,'n') << ' ' << -q(i,m-1,'e') << " 0\n";
    }
